...
namespace Sakaji_Hajar_a3_project
{
    internal class Movement
    {
        ...
        {
            for (int startingPosition = 0; startingPosition < enemies.Length; startingPosition++)
            {
                enemies[startingPosition].X -= speed;

                if (enemies[startingPosition].X < -characterSize)
                {
                    enemies[startingPosition].X = screenWidth + rand.Next(100, 300);
                }
            }
        }
